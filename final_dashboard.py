...
with st.sidebar:
    ...
    selected_columns = st.multiselect(
        ...
        default=default_columns or columns_in_table[:1],
    )
    selected_group = ", ".join(selected_columns) if selected_columns else "None"
...
raw_frame = rows_to_frame(selected_table, selected_columns)
if raw_frame.empty:
...
metric_3.metric("Selected", selected_group)

weighted_total = selected_table.get("data", {}).get("Weighted Sample", {}).get("Total")
metric_4.metric("Sample Base", f"{weighted_total:,.0f}" if isinstance(weighted_total, (int, float)) else "NA")

if chart_type == "Pie" and len(selected_columns) > 1:
    st.info(f"Pie chart is showing {selected_columns[0]}. Choose only one top break for a different pie.")
...
st.divider()
st.subheader("Data")
sort_column = "Total" if "Total" in wide_frame.columns else selected_columns[0]
